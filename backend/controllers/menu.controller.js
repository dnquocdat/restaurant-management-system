...
export const searchMenu = async (req, res, next) => {
    ...
    if (sort) {
        ...
        if (!field || !direction || !['dish_name', 'price'].includes(field.toLowerCase()) || !['asc', 'desc'].includes(direction.toLowerCase())) {
            throw new CustomError("BAD_REQUEST", "Invalid sort parameter format. Use sort=field,asc|desc", STATUS_CODE.BAD_REQUEST);
        }
    }
    
    const { listDish, totalRecords } = await searchMenuService({branch_id,  query, category, page, limit, field, direction});
