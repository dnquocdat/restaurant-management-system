...
export const updateUser = async (req, res, next) => {
    ...
    if (user_phone_number !== undefined) updateData.user_phone_number = user_phone_number;
...
export const updateUserPassword = async (req, res, next) => {
    ...
    if (!current_password || !new_password) {
        throw new CustomError("BAD_REQUEST", "Please provide both current and new passwords", STATUS_CODE.BAD_REQUEST);
    }

    // Retrieve the user's current hashed password from the database
    const userCheckSql = 'SELECT user_password FROM online_account WHERE user_id = ? LIMIT 1';
    const [existingRows] = await db.query(userCheckSql, [userId]);

    if (existingRows.length === 0) {
        throw new CustomError("NOT_FOUND", "User does not exist", STATUS_CODE.NOT_FOUND);
    }
    ...
    if (!isMatch) {
        throw new CustomError("BAD_REQUEST", "Current password is incorrect", STATUS_CODE.BAD_REQUEST);
    }
