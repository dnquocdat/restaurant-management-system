...
export function test(req, res) {
    throw new CustomError('TEST_ERROR', 'This is a test error', 400, {user: req.user});
}
...
export const loginUser = async (req, res) => {
    ...
    if (existingRows.length === 0) {
        throw new CustomError("BAD_REQUEST", "User doesn't exist", STATUS_CODE.BAD_REQUEST);
    }
    ...
    if (!match)
        throw new CustomError("BAD_REQUEST", "Invalid credentials", STATUS_CODE.BAD_REQUEST);
...
export const requestRefreshToken = async (req, res) => {
    ...
    if (!refreshToken) {
        throw new CustomError("UNAUTHORIZED", "Unauthorized!", STATUS_CODE.UNAUTHORIZED);
    }
...
export const logoutUser = async (req, res) => {
    ...
    res.clearCookie("refreshToken");
    res.clearCookie("tokenLogout");
    return formatResponse(res, "Success", "User logged out successfully", STATUS_CODE.SUCCESS, {});
};